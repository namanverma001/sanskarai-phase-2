<div class="page-header">
    <h1 class="page-title">Rate Your Order</h1>
    <p class="text-muted">Share your feedback about <?= htmlspecialchars($order['vendor_name']) ?></p>
</div>

<div class="content-grid">
    <div class="card">
        <div class="card-header">
            <h3 class="card-title"><i class="fas fa-star"></i> Review Vendor</h3>
        </div>
        
        <div class="order-info" style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 25px;">
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
                <div>
                    <strong>Vendor:</strong> <?= htmlspecialchars($order['vendor_name']) ?>
                </div>
                <div>
                    <strong>Order #:</strong> <?= htmlspecialchars($order['order_number'] ?? $order['id']) ?>
                </div>
                <div>
        ...
        <form id="vendorReviewForm" action="/user/reviews/vendor" method="POST">
            ...
            <div class="rating-categories" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-bottom: 25px;">
                <div class="form-group">
                    ...
                    <div class="star-rating-input small" data-field="item_quality">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                        <span class="star" data-value="<?= $i ?>"><i class="far fa-star"></i></span>
                        <?php endfor; ?>
                    </div>
                    <input type="hidden" name="item_quality" id="item_quality">
                </div>

                <div class="form-group">
                    <label class="form-label">On-Time Delivery</label>
                    <div class="star-rating-input small" data-field="delivery_time">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                        <span class="star" data-value="<?= $i ?>"><i class="far fa-star"></i></span>
                        <?php endfor; ?>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="star-rating-input small" data-field="packaging">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                        <span class="star" data-value="<?= $i ?>"><i class="far fa-star"></i></span>
                        <?php endfor; ?>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="star-rating-input small" data-field="value_for_money">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                        <span class="star" data-value="<?= $i ?>"><i class="far fa-star"></i></span>
                        <?php endfor; ?>
                    </div>
                    <input type="hidden" name="value_for_money" id="value_for_money">
                </div>
            </div>
            ...
            <div class="form-actions" style="display: flex; gap: 15px;">
                ...
                <a href="/user/orders" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (result.success) {
                // Show success modal
                showSuccessModal(result.message, result.redirect || '/user/orders');
            } else {
                if (result.errors) {
